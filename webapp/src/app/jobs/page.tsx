...
function deriveTargetRole(analysisResult: Record<string, unknown> | null): string {
  ...
  if (allSkills.some((s) => ["pytorch", "tensorflow", "machine learning", "deep learning"].includes(s)))
  ...
  if (allSkills.some((s) => ["fastapi", "django", "node.js", "express", "spring boot"].includes(s)))
    return "Backend Engineer";
  if (allSkills.some((s) => ["react", "next.js", "vue", "angular"].includes(s)))
    return "Frontend Engineer";
  if (allSkills.some((s) => ["kubernetes", "terraform", "aws", "devops", "docker"].includes(s)))
...
export default function JobsPage() {
  ...
  const triggerSearch = useCallback((clearExisting: boolean = true) => {
    ...
    // ── Build SSE URL with session-based resume context ─────────────────────
    // session_id identifies the user's resume in Redis (server-side).
    // target_role is derived from the user's analysis result (not hardcoded).
    ...
    if (!sessionId) {
      // FastAPI was offline when the resume was uploaded — session was not stored.
      // Show a user-friendly error rather than connecting to a broken SSE stream.
    ...
    // status: phase updates from the live pipeline (discovery, scoring)
    eventSource.addEventListener("status", (event) => {
      try {
        const status = JSON.parse(event.data) as { phase: string; message: string };
    ...
    // done: server signals clean stream completion (all jobs processed)
    ...
    eventSource.addEventListener("error", (e: Event) => {
      ...
      // Check if this is a structured error from the server
  ...
  }, [cacheJobs, closeStream]);

  const fetchJobs = useCallback(async () => {
    // Check sessionStorage cache first (valid for 15 minutes)
    ...
    // No cache — trigger the live discovery + scoring pipeline via SSE
    triggerSearch();
  }, [triggerSearch]);
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        ...
        <p>
          {loading
            ? "LangGraph pipeline is searching for the best opportunities..."
      ...
      {loading ? (
        <div>
          {/* Analysis animation for job search */}
          <div className="analysis-container">
            <div style={{ textAlign: "center", marginBottom: 32 }}>
              ...
              <p style={{ color: "var(--text-secondary)" }}>
                {statusMessage || "LangGraph pipeline is scouring the web for matching opportunities"}
              </p>
            </div>
            <div className="analysis-stages">
              {[
                { icon: "🔍", title: "Search Node", desc: "Querying job boards and career pages" },
                { icon: "📋", title: "Extract Node", desc: "Scraping and structuring job requirements" },
                { icon: "🎯", title: "Match Node", desc: "Ranking jobs by profile fit" },
              ].map((s, i) => (
                <div
                  key={i}
                  className={`analysis-stage ${searching ? "active" : ""}`}
                ...
                >
                  ...
                  <div className="stage-status running">
            ...
            <AgentLogFeed active={searching || loading} />
          </div>
        </div>
      ) : error ? (
